...
#
# Examples:
#   python alden_cli.py add "Study Session" 2025-08-09T10:00 --priority high
...
def cmd_add(args):
    ev = add_event(args.title, args.time, category=args.category, priority=args.priority)
    print(f"Added: {ev['title']} @ {ev['time']} (id={ev['id']})")


def cmd_list(_args):
    for e in list_events():
        print(f"{e.get('time','?')} | {e.get('title')} "
              f"[{e.get('priority')}] ({e.get('status')}) id={e.get('id')}")
